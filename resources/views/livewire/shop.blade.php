<div>
    <div id="shop" class="mb-10 pt-4 mt-0 flex items-center gap-4 ">
        ...
        <a class="font-satoshi text-[14px] lg:text-[16px] text-black ">Shop</a>
    </div>
            ...
            <div id="filters" class="hidden lg:block w-full max-w-80 border border-black/10 rounded-[20px] overflow-y-auto">
                ...
                <div class="flex flex-col  gap-5 mt-5 mx-6 pb-8 border-b border-black/10">
                    ...
                    <div class="flex flex-wrap justify-between gap-y-3">
                        @foreach ($categories as $category)     
                            <button wire:click="selectCategory({{ $category->id }})" class="{{ in_array($category->id, $category_ids) ? 'text-white bg-black' : 'text-black/60 hover:bg-[#EAEAEA]' }}  font-satoshi text-[16px] text-black/60 bg-[#F0F0F0] py-2.5 w-[48%] rounded-[10px] cursor-pointer relative overflow-hidden">
                                {{ $category->name }}
                            </button>
                        @endforeach
                    </div>
                ...
                <div class="flex flex-col gap-10 mt-5 mx-6 pb-8 border-b border-black/10">
                    ...
                    <div wire:ignore id="slider" class="bg-[#F0F0F0] w-[100%] h-2 rounded-[20px] relative"> 
                        <span id="minPrice" class="font-satoshi font-medium text-[14px] text-center w-9  top-4 -translate-x-[50%] absolute">0$</span>
                    ...
                    <form class="flex justify-between relative font-satoshi text-[16px]">
                        <div>
                            <label>From ($)</label>
                            <input wire:model.live="minInput" type="number" id="minInput" placeholder="Min" max="460" min="0" value="0"
                ...
                <div class="flex flex-col gap-5 mt-5 mx-6 pb-8 border-b border-black/10">
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach ($sizes as $size)     
                            <button wire:click="selectSize({{ $size->id }})" class="{{ in_array($size->id, $size_ids) ? 'text-white bg-black' : 'text-black/60 hover:bg-[#EAEAEA]' }}  font-satoshi text-[16px] text-black/60 bg-[#F0F0F0] py-2.5 px-5 rounded-[62px] cursor-pointer relative overflow-hidden">
                                {{ $size->name }}
                            </button>
                        @endforeach
                    </div>
            ...
            <div class="lg:min-w-[925px] flex flex-col gap-6">
                <div class="flex justify-between items-center">
                    <div class="flex gap-4">
                        <h1 class="font-satoshi font-bold text-[20px] lg:text-[24px] lg:text-[32px] {{ $search ? '' : 'hidden' }} }}">Search results for "{{ $search }}"</h1>
                        <span class="flex items-center font-satoshi text-[14px] lg:text-[16px] text-black/60">Showing 1-9 of 100 Products</span>
                    </div>
                    <div id="filtersBtn" class="lg:hidden flex items-center justify-center bg-[#F0F0F0] w-[32px] h-[32px] rounded-full">
                ...
                <div class="flex flex-wrap justify-between w-full gap-y-8 max-w-239 pb-6 border-b border-black/10">
                    @foreach ($products as $product)    
                        ...
                        <div wire:click="show({{ $product->id }})"  class="flex flex-col w-full max-w-[45%] lg:max-w-[30%] gap-[8px] cursor-pointer">
                            ...
                            <label class="font-satoshi font-bold text-[16px] lg:text-[20px]">{{ $product->name }}</label>
                            <div class="flex gap-[5px]">
                                @for ($i = 1; $i <= $rating; $i += 0.5)
                                    @if (fmod($i, 1) == 0)
                                        <svg width="18" height="17" viewBox="0 0 18 17" fill="none"xmlns="http://www.w3.org/2000/svg"><path    d="M8.79246 0L11.4117 5.63991L17.5849 6.38809L13.0304 10.6219L14.2265 16.7243L8.79246 13.701L3.35842 16.7243L4.5545 10.6219L2.86102e-06 6.38809L6.17325 5.63991L8.79246 0Z"    fill="#FFC633" /></svg>
                                    @elseif ($i == $rating)
                                        <svg width="9" height="17" viewBox="0 0 9 17" fill="none"xmlns="http://www.w3.org/2000/svg"><path    d="M3.35842 16.7243L8.79246 13.701V0L6.17325 5.63991L0 6.38809L4.55449 10.6219L3.35842 16.7243Z"    fill="#FFC633" /></svg>
                                    @endif
                                @endfor                                
