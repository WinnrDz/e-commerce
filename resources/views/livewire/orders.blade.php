<div>
    ...
    <h1 class="font-integral font-bold text-[40px] w-[100%] mb-10">Your Orders</h1>
    <div class="flex flex-col items-center justify-center gap-y-8">
        @foreach($orders as $order)
            <div class="flex flex-col items-center py-5 px-8 gap-6 w-full  border border-black/10 rounded-[20px]">
                <div class="flex justify-between w-full">
                    <div>
                        <h2 class="font-satoshi font-bold text-[16px] lg:text-[20px]">Order #{{ $order->id }}</h2>
                        <span class="font-satoshim text-[16px] text-black/60">{{ $order->created_at->format('F d, Y') }} &nbsp; • &nbsp;{{ $order->itemsCount() }}</span>
                    </div>
                ...
                <div class="flex flex-row w-full">
                    <div class="flex flex-row items-center gap-4 border-r-3 border-black/10 w-full max-w-[60%]">
                        @foreach ($order->variants->unique('product_id') as $variant)    
                            ...
                            @endphp
                            <span class="font-satoshi font-bold text-[16px]">x {{ $quantity  }}</span>
                        @endforeach

                    </div>
                    <div class="flex justify-between w-full max-w-[40%] pl-20">
